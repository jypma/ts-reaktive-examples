...
public class ConversationProtocol {
    ...
    public CompletionStage<Done> postMessage(String conversationId, String message) {
        return ask(supervisor, new ConversationSupervisor.RoutedMessage(conversationId,
            ConversationCommand.postMessage(message)), 5000
    ...
    public CompletionStage<Seq<String>> getMessages(String conversationId) {
        return ask(supervisor, new ConversationSupervisor.RoutedMessage(conversationId,
            ConversationCommand.getMessageList()), 5000
        ).thenApply(l -> (Seq<String>) l);
    }
}
